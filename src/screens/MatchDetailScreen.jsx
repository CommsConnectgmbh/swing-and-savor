...
export default function MatchDetailScreen() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      channelRef.current?.(); channelRef.current = null
    }
  ...
  function subscribeRealtime(token) {
    channelRef.current?.()
    channelRef.current = subscribeToTables(`match-${matchId}`, [
      { table: 'hole_results', filter: `match_id=eq.${matchId}`, handler: ({ new: hr }) => {
        if (token !== loadTokenRef.current || !hr?.hole_number) return
        mergeRemoteHole(hr)
      } },
      { event: 'UPDATE', table: 'matches', filter: `id=eq.${matchId}`, handler: ({ new: m }) => {
